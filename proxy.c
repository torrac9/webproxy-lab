...
void doit(int clientfd)
{
  ...
  // Server 소켓 생성
  serverfd = is_local_test ? Open_clientfd(hostname, port) : Open_clientfd("3.37.62.158", port);
  if (serverfd < 0)
  {
  ...
  /* 2️⃣ Request Header 읽기 & 전송 [ Client -> Proxy -> Server ] */
  read_requesthdrs(&request_rio, request_buf, serverfd, hostname, port);

  /* 3️⃣ Response Header 읽기 & 전송 [ Server -> Proxy -> Client ] */
