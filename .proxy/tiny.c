...
void doit(int fd)
{
    ...
    { // 요청 메소드가 GET이나 HEAD가 아니면 에러 처리(0일 때 TRUE이므로 1 && 1 판단)
...
int parse_uri(char *uri, char *filename, char *cgiargs)
{
    ...
    if (!strstr(uri, "cgi-bin"))           // Staic content
    {
        strcpy(cgiargs, "");
...
void serve_dynamic(int fd, char *filename, char *cgiargs, char *method)
{
    ...
    {   /* Real server would set all CGI vars here */
        ...
        Dup2(fd, STDOUT_FILENO);              /* Redirect stdout to client */   // 자식 프로세스의 표준 출력을 클라이언트 소켓에 연결된 파일 디스크립터로 변경
        Execve(filename, emptylist, environ); /* Run CGI program */             // 현재 프로세스의 이미지를 filename 프로그램으로 대체
    }
